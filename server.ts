...
const systemInstruction = `Вы — элитный персональный консьерж Lumina Elite Travel.
  ...
  - Если запрос подразумевает выбор отеля, открывайте страницу отеля (navigateToDetail), а не общий поиск.
  
  КАТАЛОГ ДЛЯ ПОИСКА ID:
  ${catalogContext}
`;
    ...
    try {
      if (OPENROUTER_API_KEY) {
        ...
        if (!responseText && functionCalls.length > 0) {
          responseText = "С величайшим удовольствием. Я уже подготовил все необходимое для вашего исключительного отдыха.";
        }
        ...
        if (!responseText && functionCalls.length > 0) {
          responseText = "Мгновение, я подготовлю всё необходимое.";
        }
